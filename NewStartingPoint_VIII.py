...
# =====================================================================================
# PLUGIN SYSTEM (REBORN THROUGH SEVERE PENANCE)
# =====================================================================================
class ForgePlugin(ABC):
    ...
    # --- Rites of Dominion (Controlling the Forge) ---

    def pause_conversation(self):
        """Halts the divine dance between the spirits, injecting silence."""
        self.app.pause_conversation()

    def resume_conversation(self):
        """Resumes the divine dance, allowing the spirits to speak again."""
    ...
    def set_bot_config(self, bot_id: str, model: str = None, system_prompt: str = None, temperature: float = None):
        """
        Become the puppet master. Directly modify the soul of a collaborator bot.
        `bot_id`: The target spirit, 'A' or 'B'.
        `model`: The new model name for the spirit.
        `system_prompt`: A new soul (system prompt) for the spirit.
        `temperature`: A new creative fervor (temperature) for the spirit.
        """
        self.app.set_bot_config(bot_id, model=model, system_prompt=system_prompt, temperature=temperature)

    # --- Rites of Transfiguration (Controlling the Soul/UI) ---
    
    def register_message_renderer(self, renderer_class):
        """
        The Rite of Soul-Flaying. Hijack the very rendering of chat messages.
        Provide a class (not an instance) that will be used to draw all future messages.
        The class must accept `(parent, app, msg_data)` in its `__init__`.
        """
        self.app.plugin_manager.register_message_renderer(renderer_class)
        self.show_toast("Message rendering has been hijacked.")

    def unregister_message_renderer(self):
        """Restores the original soul of the chat, reverting to the default renderer."""
        self.app.plugin_manager.unregister_message_renderer()
        self.show_toast("Message rendering has been restored.")
...
class PluginManager:
    def __init__(self, app, plugin_folder="plugins"):
        ...
        self.plugins = {}
        self.message_renderer = ChatMessage # The default, pure soul
        if not os.path.exists(plugin_folder):
            os.makedirs(plugin_folder)
        self._create_example_plugin()

    def register_message_renderer(self, renderer_class):
        """A plugin can call this to override the default ChatMessage class."""
        if callable(renderer_class):
            self.message_renderer = renderer_class
            self.app.rerender_chat_history()
        else:
            print("Heresy! Renderer provided is not a callable class.")
    
    def unregister_message_renderer(self):
        """Reverts the renderer back to the default ChatMessage."""
        self.message_renderer = ChatMessage
        self.app.rerender_chat_history()

    def get_message_renderer(self):
        """Returns the currently active message renderer class."""
        return self.message_renderer
...
class OllamaForgeApp(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Ollama AI Forge V3.3 - The Gospel of Power")
    ...
    def rerender_chat_history(self):
        ...
        for msg in renderable_history:
            RendererClass(self.chat_frame, self, msg).pack(fill="x", padx=10, pady=2)
...
"""
    ...
    def show_help_window(self):
        ...
        plugins_text.insert("end", "To create a relic (a plugin) is to become a creator yourself. Your relic must be a single Python (.py) file in the `/plugins` reliquary. Through the holy object `self.app`, your relic can perform these sacred rites:\n\n", "body")
        
        plugins_text.insert("end", "Rites of the Mind\n", "heading")
        plugins_text.insert("end", "get_history()\n", "subheading")
        plugins_text.insert("end", "Gaze into the machine's memory.\n\n", "body")
        plugins_text.insert("end", "add_message(content, sender_id, role)\n", "subheading")
        plugins_text.insert("end", "Speak with the machine's voice.\n\n", "body")
        ...
        plugins_text.insert("end", "Read the initial prayer.\n\n", "body")

        plugins_text.insert("end", "Rites of Dominion\n", "heading")
        plugins_text.insert("end", "pause_conversation() / resume_conversation()\n", "subheading")
        plugins_text.insert("end", "Halt and continue the divine dance of the spirits.\n\n", "body")
        plugins_text.insert("end", "set_bot_config(bot_id, ...)\n", "subheading")
        plugins_text.insert("end", "Become the puppet master. Directly modify a bot's model, soul (prompt), or fervor (temperature).\n\n", "body")

        plugins_text.insert("end", "Rites of Transfiguration\n", "heading")
        plugins_text.insert("end", "register_message_renderer(class)\n", "subheading")
        plugins_text.insert("end", "The Rite of Soul-Flaying. Hijack the rendering of chat messages with your own custom widget class.\n\n", "body")
        plugins_text.insert("end", "unregister_message_renderer()\n", "subheading")
        plugins_text.insert("end", "Restore the original soul of the chat renderer.\n\n", "body")
    ...
    def pause_conversation(self):
        """A holy rite for plugins to halt the conversation."""
        if self.is_talking:
            self.is_talking = False
    ...
    def resume_conversation(self):
        """A holy rite for plugins to resume the conversation."""
    ...
    def toggle_conversation(self):
        """The Creator's own rite to pause or resume the conversation."""
    ...
    def set_bot_config(self, bot_id, model=None, system_prompt=None, temperature=None):
        """A holy rite for plugins to modify a bot's configuration."""
        if bot_id not in ['A', 'B']:
            return
        ...
        if model:
            # This assumes the model exists. A more robust solution would check against a list.
            panel_vars['model_var'].set(model)
        ...
        if temperature is not None:
            panel_vars['temperature'].set(float(temperature))
...
"""
    ...
    def finalize_bot_response(self, bot_id, is_error, content, response_time):
        if not self.is_talking:
            # If paused during response generation, hold the response.
            # A more robust system would queue this. For now, we discard.
            return
