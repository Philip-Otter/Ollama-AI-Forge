...
class ChatMessage(ttk.Frame):
    """The default vessel for the spirits' words. Its form is perfect and eternal."""
    def __init__(self, parent, app, msg_data, **kwargs):
        ...
        # The flesh of the message pulses into existence
        self.app.after(10, lambda: self.app.animation_engine.flesh_pulse(self.bubble_frame))
    ...
    def _render_message(self):
        ...
        sender_key = self.sender.lower().replace(' ', '_')
        if not (self.is_bot or self.is_human or sender_key == 'system'):
            sender_key = 'plugin' 
        
        avatar_color = self.theme.get(f"{sender_key}_color", self.theme['fg'])
        ...
        avatar_canvas.create_text(16, 16, text=avatar_initial, font=self.app.bold_font, fill=self.theme.get('bg', '#FFFFFF'))
        avatar_canvas.grid(row=0, column=avatar_col, sticky="ne" if self.is_human else "nw", padx=5, pady=5)
        
        self.bubble_frame = ttk.Frame(self, style="Bubble.TFrame")
        self.bubble_frame.grid(row=0, column=bubble_col, sticky="ew", padx=5)
        
        header_frame = ttk.Frame(self.bubble_frame, style="Bubble.TFrame"); header_frame.pack(fill="x", padx=(10, 5), pady=(5, 0))
        ...
        content_frame = ttk.Frame(self.bubble_frame, style="Bubble.TFrame")
    ...
    def add_text_segment(self, parent, text):
        text_widget = tk.Text(parent, wrap="word", relief="flat", highlightthickness=0, bg=self.theme['select_bg'], fg=self.theme['fg'], font=self.app.default_font, borderwidth=0, state="disabled", height=1, padx=5, pady=5)
